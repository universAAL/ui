...
public class SmartPendingSystemMenuProvider implements ISystemMenuProvider {
	...
	private void putPendingXXSubmits(Group stdButtons) {
		...
		if (!userDM.getMessagePool().listAllSuspended().isEmpty()) {
			new Submit(stdButtons,
					new Label(ulh.getString(MessageConstants.MENU_PROVIDER_PENDING_MESSAGES),
							ulh.getString(MessageConstants.MENU_PROVIDER_PENDING_MESSAGES_ICON)),
					MESSAGES_CALL).setHelpString(ulh.getString(MessageConstants.MENU_PROVIDER_PENDING_MESSAGES_HELP));
			// } else {
			// show a button with different ICON/Message or nothing
		}
		if (!userDM.getDialogPool().listAllSuspended().isEmpty()) {
			new Submit(stdButtons,
					new Label(ulh.getString(MessageConstants.MENU_PROVIDER_PENDING_DIALOGS),
							ulh.getString(MessageConstants.MENU_PROVIDER_PENDING_DIALOGS_ICON)),
					OPEN_DIALOGS_CALL)
							.setHelpString(ulh.getString(MessageConstants.MENU_PROVIDER_PENDING_DIALOGS_HELP));
			// } else {
			// show a button with different ICON/Message or nothing
		}
	}
