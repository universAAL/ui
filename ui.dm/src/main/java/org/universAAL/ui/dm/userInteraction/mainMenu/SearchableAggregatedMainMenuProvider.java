...
public class SearchableAggregatedMainMenuProvider extends AggregatedMainMenuProvider {
	 ...
	 */
	static final String SEARCH_CALL = DialogManagerImpl.CALL_PREFIX + "#doSearch"; //$NON-NLS-1$

	static final String BACK_CALL = DialogManagerImpl.CALL_PREFIX + "#doBackFromSearch"; //$NON-NLS-1$ ;
	...
	public SearchableAggregatedMainMenuProvider(UserDialogManager userDM) {
		super();
		this.userDM = userDM;
		if (userDM.getUiPreferencesSubProfile().getSystemMenuPreferences().getSearchFeatureIsFirst() == Status.on) {
			searchFirst = true;
		} else
			searchFirst = false;
	}
	...
	public void handle(UIResponse response) {
		...
		if (SEARCH_CALL.equals(submissionID)) {
			Object searchStr = response.getUserInput(new String[] { PROP_SEARCH_STRING });
			if (searchStr instanceof String) {
				searchString = (String) searchStr;
			} else {
				LogUtils.logError(DialogManagerImpl.getModuleContext(), getClass(), "handle",
	...
	private int filter(Group mmGroup) {
		...
		for (int i = 0; i < elem.length; i++) {
			if (!elem[i].getLabel().getText().toLowerCase().contains(searchString.toLowerCase())
	...
	public Group getMainMenu(Resource user, AbsLocation location, Form systemForm) {
		if (searchString == null) {
			...
			new Submit(mmGroup,
					new Label(userDM.getLocaleHelper()
							.getString(MessageConstants.SEARCHABLE_AGGREGATED_MAIN_MENU_PROVIDER_BACK), null),
					BACK_CALL);
			return mmGroup;
		}
	}
	...
	private Group addSearch(Group parent) {
		...
		Group g = new Group(parent,
				new Label(ulh.getString(MessageConstants.SEARCHABLE_AGGREGATED_MAIN_MENU_PROVIDER_SEARCH), null), null,
				null, null);
		InputField in = new InputField(g, null, new PropertyPath(null, false, new String[] { PROP_SEARCH_STRING }),
				MergedRestriction.getAllValuesRestrictionWithCardinality(PROP_SEARCH_STRING,
						TypeMapper.getDatatypeURI(String.class), 1, 1),
				null);
		in.setMaxLength(20);
		new Submit(g, new Label(ulh.getString(MessageConstants.SEARCHABLE_AGGREGATED_MAIN_MENU_PROVIDER_SEARCH), null),
