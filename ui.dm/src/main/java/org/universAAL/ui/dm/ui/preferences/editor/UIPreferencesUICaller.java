...
public class UIPreferencesUICaller extends UICaller {
	...
	public UIPreferencesUICaller(ModuleContext mcontext, IUIPreferencesBuffer uiPreferencesBuffer) {
		super(mcontext);
	...
	public void handleUIResponse(UIResponse uiResponse) {
		...
		try {
			if (submit.equals("submit_uiPreferences")) {
				...
				LogUtils.logDebug(mcontext, this.getClass(), "handleUIResponse",
						new Object[] { "Handling edit ui preferences submit" }, null);
				...
				LogUtils.logWarn(mcontext, this.getClass(), "handleUIResponse",
						new Object[] { "We shouldnt have got here, with submit: ", submit }, null);

				submit = null;
			}
		} catch (Exception e) {
			LogUtils.logError(mcontext, getClass(), "handleUIResponse",
					new String[] { "Unable to update UIPreferencesProfile" }, e);

			if (addressedUser != null) {
				UserDialogManager udm = DialogManagerImpl.getInstance().getUDM(addressedUser.getURI());
				if (udm != null) {
					MessageLocaleHelper mlocaleHelper = udm.getLocaleHelper();
					showMessageScreen(addressedUser,
							mlocaleHelper.getString("UIPreferencesUICaller.UnknownServiceError"));
				} else {
	...
	public void showMessageScreen(User user, String msg) {
		LogUtils.logInfo(mcontext, this.getClass(), "showMessageScreen",
				new Object[] { "Sending UI Request with info message: " + msg + " screen for user {}", user.getURI() },
				null);
		Form f = uiPreferencesDialogBuilder.getStatusMessageForm(msg);
		UIRequest uiReq = new UIRequest(user, f, LevelRating.middle, Locale.ENGLISH, PrivacyLevel.insensible);
		sendUIRequest(uiReq);
	}
	...
	protected void handleSubmit(UIResponse uiResponse) {
		LogUtils.logDebug(mcontext, this.getClass(), "handleSubmit",
				new Object[] { "Processing Input: submit_uiPreferences" }, null);
		...
		try {
			...
			try {
				langLabel = updatedUIPreferencesSubProfile.getInteractionPreferences().getPreferredLanguage()
						.getNativeLabel();
			} catch (Exception e) {
				LogUtils.logError(mcontext, this.getClass(), "handleSubmit",
						new Object[] {
			...
			if (langLabel != null && !langLabel.equalsIgnoreCase(System.getProperty("user.language")))
				...
				LogUtils.logWarn(mcontext, this.getClass(), "handleSubmit",
						new Object[] {
								"JVM user.language property not refreshed with UI Preferences subprofile. Old value remained." },
						null);

			// refresh buffer with new data
			uiPreferencesBuffer.changeCurrentUIPreferencesSubProfileForUser(addressedUser,
					updatedUIPreferencesSubProfile);

		} catch (Exception e) {
			LogUtils.logError(mcontext, this.getClass(), "handleSubmit",
					new String[] { "Unknown error processing the user input" }, e);
	...
	public void showUIPreferencesEditorScreen(User user) {
		LogUtils.logInfo(mcontext, this.getClass(), "showInitialScreen",
				new Object[] { "Sending UI Request: showUIPreferencesEditorScreen for user:" + user.getURI() }, null);
