 ...
 * location info to all {@link UIRequest}s for that user. It adds the last user
 * location as presentation location. This location may not be added if the
...
public class AdapterUserLocation extends ContextSubscriber implements IAdapter {
	...
	private static ContextEventPattern[] getPermanentSubscriptions(Resource forUser) {
		ContextEventPattern contextEventPattern = new ContextEventPattern();
		contextEventPattern
				.addRestriction(MergedRestriction.getFixedValueRestriction(ContextEvent.PROP_RDF_SUBJECT, forUser));
		...
		if (user.getURI().equals(event.getRDFSubject().getURI())) {
			...
			if (!newUserLocation.equals(userLocation)) {
				...
				if (udm != null) {
					...
					LogUtils.logError(mcontext, getClass(), "handleContextEvent",
							"unable to locate UDM for user: " + user.getURI());
				}
			}
			...
			long clearLocationPeriod = Long
					.parseLong(System.getProperty(SYSTEM_PROP_CLEAR_LOCATION_PERIOD, DEFAULT_CLEAR_LOCATION_PERIOD));
			task = sched.schedule(new ClearLocationTask(), clearLocationPeriod, TimeUnit.MILLISECONDS);
		} else {
			LogUtils.logDebug(mcontext, getClass(), "handleContextEvent",
					new Object[] { "\nReceived context event carrying location for user: " + event.getRDFSubject()
							+ "\n instead for user " + user.getURI() + " so discarrding the event." },
					null);
		}
	...
	public void adapt(UIRequest uiRequest) {
		...
		if (userLocation != null) {
			...
			LogUtils.logInfo(mcontext, getClass(), "adapt",
					new String[] { "Setting user location as presentation location: " + userLocation.getURI() }, null);
		}
	}
	...
	class ClearLocationTask implements Runnable {
		public void run() {
			...
			LogUtils.logDebug(mcontext, getClass(), "run",
					new String[] { "ClearLocationTask finished for user: " + user.getURI() }, null);
		}
	}
}
