...
public class SCallee extends ServiceCallee {
	...
	static {
		...
		new ProfilingService(SRV_ADD_ENTRY) {
			void init() {
				addFilteringInput(IN_USER_ID, User.MY_URI, 1, 1, new String[] { ProfilingService.PROP_CONTROLS });
		...
		new ProfilingService(SRV_GET_ENTRIES) {
			void init() {
				addFilteringInput(IN_USER_ID, User.MY_URI, 1, 1, new String[] { ProfilingService.PROP_CONTROLS });
				addOutput(OUT_ENTRIES, MenuEntry.MY_URI, 0, 0, new String[] { ProfilingService.PROP_CONTROLS,
						Profilable.PROP_HAS_PROFILE, Profile.PROP_HAS_SUB_PROFILE, MenuProfile.PROP_ENTRY });
	...
	private ServiceResponse getEntries(User user) {
		try {
			...
			LogUtils.logError(context, SCallee.class, "getEntries",
					new Object[] { "an error occured while trying to get the menu entries." }, e);
			return new ServiceResponse(CallStatus.serviceSpecificFailure);
		}
	}

	private ServiceResponse addEntry(User user, MenuEntry entry) {
		if (entry.getPath() == null || entry.getPath().size() <= 0 || entry.getClassURI() == null
				|| entry.getClassURI().isEmpty() || entry.getServiceClass() == null) {
			// invalid entry
			LogUtils.logError(context, getClass(), "addEntry", createLogMessage("Illegal MenuEntry ", user, entry),
					null);
			return new ServiceResponse(CallStatus.serviceSpecificFailure);
		}
		...
		try {
			...
			LogUtils.logDebug(context, SCallee.class, "addEntry",
					new Object[] { "adding main menu entry: ", entry.getServiceClass().getURI() }, null);
			refreshMainMenu(user.getURI());
			return new ServiceResponse(CallStatus.succeeded);
		} catch (Exception e) {
			LogUtils.logError(context, SCallee.class, "addEntry",
					new Object[] { "an error occured while trying to add a new menu entry: " }, e);
			return new ServiceResponse(CallStatus.serviceSpecificFailure);
		}
	}

	private ServiceResponse removeEntry(User user, MenuEntry entry) {
		// remove the menu entry: load the contents of the config file, remove
		// the
		// entry, and then save the file again
		try {
			RDFMainMenu mainMenu = new RDFMainMenu(context);
			...
			if (r == null) {
				LogUtils.logDebug(context, SCallee.class, "removeEntry",
						createLogMessage("trying to remove the main menu entry ", user, entry,
			...
			mainMenu.saveMenu(getFile(user.getURI()), mp);
			LogUtils.logDebug(context, SCallee.class, "removeEntry",
					createLogMessage("main menu entry removed: ", user, entry), null);
			refreshMainMenu(user.getURI());
			return new ServiceResponse(CallStatus.succeeded);
		} catch (Exception e) {
			LogUtils.logError(context, SCallee.class, "removeEntry",
					createLogMessage("an error occured while trying to remove the menu entry: ", user, entry), e);
			return new ServiceResponse(CallStatus.serviceSpecificFailure);
		}
	}
	...
	private Object[] createLogMessage(String msg, User user, MenuEntry entry) {
		return new Object[] { msg, entry.getServiceClass(), " from vendor ", entry.getVendor(), " for user ",
				user.getURI() };
	}

	private Object[] createLogMessage(String msg, User user, MenuEntry entry, String suffix) {
		return new Object[] { msg, entry.getServiceClass(), " from vendor ", entry.getVendor(), " for user ",
				user.getURI(), suffix };
	}
}
