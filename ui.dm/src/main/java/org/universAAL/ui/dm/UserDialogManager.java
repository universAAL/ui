...
public class UserDialogManager implements IDialogManager, IUIPreferencesChangeListener {
	...
	void close() {
		if (uiPreferencesSubProfile != null
				&& uiPreferencesSubProfile.getSystemMenuPreferences().getUIRequestPersistance().equals(Status.on)) {
			saverTask.cancel();
			saverTask.write();
		}
	}
	...
	public void changedUIPreferences(UIPreferencesSubProfile subProfile) {
		if (subProfile == null) {
			LogUtils.logWarn(DialogManagerImpl.getModuleContext(), getClass(), "changedUIPreferences",
		...
		adapterList.add(new AdapterUserLocation(DialogManagerImpl.getModuleContext(), user));
		...
		try {
			...
			LogUtils.logError(DialogManagerImpl.getModuleContext(), getClass(), "UserDialogManager",
					new String[] { "Cannot initialize FileMainMenuProvider!" }, e);
		}
		try {
			((AggregatedMainMenuProvider) mainMenuProvider).add(new ProfilableFileMainMenuProvider(this));
		} catch (Exception e) {
			LogUtils.logError(DialogManagerImpl.getModuleContext(), getClass(), "UserDialogManager",
					new String[] { "Cannot initialize ProfilableMainMenuProvider!" }, e);
		}

		if (System.getProperty(SYSTEM_PROP_LOAD_ALL_SERVICES) != null) {
			try {
				((AggregatedMainMenuProvider) mainMenuProvider).add(new AllMainMenuProvider(user));
			} catch (Exception e) {
				LogUtils.logError(DialogManagerImpl.getModuleContext(), getClass(), "UserDialogManager",
						new String[] { "Cannot initialize AllMainMenuProvider!" }, e);
			}
		}
		 ...
		 */
		MainMenuConfigurationType mmct = uiPreferencesSubProfile.getSystemMenuPreferences().getMainMenuConfiguration();
		 ...
		 */
		PendingDialogsBuilderType pdbt = uiPreferencesSubProfile.getSystemMenuPreferences().getPendingDialogBuilder();
		 ...
		 * Persistence Setting
		 */
		if (uiPreferencesSubProfile.getSystemMenuPreferences().getUIRequestPersistance().equals(Status.on)) {
		...
		if (myUIRequests.contains(request.getDialogID())) {
			...
			if (request.getDialogType().equals(DialogType.message)) {
				...
				if (isReady && current != null) {
					...
					/*
					 * only current dialog is suspended, if there is a current
					 * message then it has to remain active, so it appears again
					 * after this more priority message is shown.
					 */
				}
				...
				isReady = dialogPool.hasToChange() && (messagePool.getCurrent() == null);
				if (isReady && current != null) {
					dialogPool.suspend(current.getDialogID());
				}
	...
	private void addSystemMenu(UIRequest request) {
		...
		if (request != null && request.getDialogForm() != null) {
			...
			if (f.getStandardButtons() != null) {
				f.getStandardButtons().changeProperty(Group.PROP_CHILDREN, null);
			}
	...
	public void showSomething() {
		try {
			LogUtils.logDebug(DialogManagerImpl.getModuleContext(), getClass(), "showSomething",
					new String[] { "Nothing to show, Trying to show something" }, null);
		} catch (Exception e) {
		...
		if (!messagePool.listAllActive().isEmpty() && messagePool.getCurrent() == null) {
			// show next message
			LogUtils.logDebug(DialogManagerImpl.getModuleContext(), getClass(), "showSomething",
					"a pending message will be shown");
			...
			LogUtils.logDebug(DialogManagerImpl.getModuleContext(), getClass(), "showSomething",
					"a pending dialog will be shown");
			...
			LogUtils.logDebug(DialogManagerImpl.getModuleContext(), getClass(), "showSomething",
					"a suspended dialog will be shown");
			resumeUIRequest(dialogPool.getNextUIRequest());
			...
			LogUtils.logDebug(DialogManagerImpl.getModuleContext(), getClass(), "showSomething",
					"there is already a current dialog shown, not showing anything.");
		}
	}
	...
	public final void resumeUIRequest(UIRequest req) {
		if (req != null) {
			...
			makeAdaptations(req);
			LogUtils.logDebug(DialogManagerImpl.getModuleContext(), getClass(), "resumeUIRequest",
					new String[] { "Resuming UIRequest:", req.getDialogForm().getTitle() }, null);
			DialogManagerImpl.getInstance().resumeDialog(req.getDialogID(), req.getDialogForm().getData());
		}
	}
	...
	private void addListeners(UIRequest req) {
		if (req != null && req.getDialogForm() != null) {
			...
			LogUtils.logWarn(DialogManagerImpl.getModuleContext(), getClass(), "addListeners",
					new String[] { "UIRequest or Form is null!" }, null);
		}
	}

	/** {@inheritDoc} */
	public final synchronized void userLogIn(Resource user, AbsLocation location) {
	...
	public void handleUIResponse(UIResponse response) {
		...
		if (listeners.containsKey(submissionID)) {
			...
			LogUtils.logWarn(DialogManagerImpl.getModuleContext(), getClass(), "handle",
					new String[] { "listeners don't include:", submissionID }, null);
	...
	public final void pushDialog(Form form) {
		...
		UIRequest req = new UIRequest(user, form, LevelRating.none,
				messageLocaleHelper.getUserLocaleFromPreferredLanguage(), PrivacyLevel.insensible);
		addSystemMenu(req);
	...
	public final void showMainMenu() {
		Form mmf = Form.newSystemMenu(messageLocaleHelper.getString("UserDialogManager.universaalMainMenu"));
	...
	public void adaptationParametersChanged(String parameter) {
		if (current != null) {
			...
			LogUtils.logDebug(DialogManagerImpl.getModuleContext(), getClass(), "adaptationParametersChanged",
					new String[] { "updating UIRequest:", current.getDialogForm().getTitle() }, null);
	...
	class ClosingTask implements Runnable {
		...
		public void run() {
			...
			// }
			try {
				Thread.sleep(Long.parseLong(System.getProperty(SYSTEM_PROP_FINALIZE_WAIT, DEFAULT_FINALISE_WAIT)));
			} catch (InterruptedException e) {
				// do nothing
			}
			try {
	...
	class AutoSaverTask extends TimerTask {
		...
		public AutoSaverTask() {
			...
			storeDLG = new DialogPoolFileStorage(DialogManagerImpl.getModuleContext(), getDialogFile());

			storeMSG = new DialogPoolFileStorage(DialogManagerImpl.getModuleContext(), getMessageFile());
		}
		...
		public void activate() {
			...
			persistencyTimer.schedule(new AutoSaverTask(), AUTOSAVE_PERIOD, AUTOSAVE_PERIOD);
		}
