...
public class MenuNode {
	...
	private class NodeChildren implements Iterable<MenuNode>, Iterator<MenuNode> {
		...
		public boolean hasNext() {
			return i < MenuNode.this.child.length && MenuNode.this.child[i] != null;
		}
	...
	/**
	 * The label of the node. This must be a simple human-readable text because
	 * it is directly shown in the user interface.
	 */
	private String label;

	/**
	 * The vendor. Multiple implementations of the same service class can be
	 * distinguished by the vendor name.
	 */
	...
	public MenuNode(int level) {
		child = new MenuNode[] { null, null, null, null, null, null, null, null, null, null, null, null, null, null,
				null };
		label = "";
		this.level = level;
	}
	 ...
	 * labels (separated by the symbol '/') that will result in multiple levels
	...
	public ServiceRequest getService(Resource user) {
		return hasService() ? InitialServiceDialog.getInitialDialogRequest(serviceClass, vendor, user) : null;
	}
	...
	// * active, has the user sufficient rights to use the Service represented
	// by
	// * this node
	// *
