...
public class RendererGuiConstants {
	...
	private static boolean WRAP_SUBMIT_TEXT = true; //shows each word of the submit's text in a separate row
	private static int WRAP_SUBMIT_INCREASE_WIDTH = 30; //increase_width and increase_height are set only in General and form properties, not in special properties like FormName.FormArea.LevelX.property
	private static int WRAP_SUBMIT_INCREASE_HEIGHT = 20; //First of all those two consts are used in buttons where the bt_white.png is used which when is stretched to a bigger image, results in the text of the button displayed over the shadow.
	private static int SELECT_VISIBLE_ROWS = 5;
	private static boolean STRETCH_SELECT_TO_MAX = true; 	//Increases the width of the combobox in order to cover all the available space between combobox's left and viewPort's (containing the combobox) right, 
															//so as not to show the horizontal scroll bar. 
															//It is useful in case the combobox items are too long and there are not other components right of combobox to be shown
	...
	private static final int DEFAULT_ROWS = 0; //is used only when DEFAULT_COLS is not set.
	...
	public RendererGuiConstants(){
		...
		//loads properties starting with General. (initially the same default values which are assigned there, but giving users the ability to change those values using the properties file)
	...
	private Properties readProperties(String propertiesFileName){
	    try{
	    	  ...
	    	  if(propertiesFile.exists()){
	    		  ...
	    		  System.out.println("ERROR in Gui.Handler: File not found! " + propertiesFileName);
	    		  return null;
	      }
	...
	public void loadLayoutConstants(String formTitle){
		try
		{
			if (readProperties(System.getProperty("user.home") + FORM_LAYOUT_PROTOTYPES_FILENAME).containsKey(formTitle.replace(" ", "").replace(":", "")))
			{
//				if ((!(formTitle.equals("General"))) && (!generalPropertiesRed))
					initializeVars();
//				else
//					generalPropertiesRed = true;
				
				this.formLayoutPrototype = (String)readProperties(System.getProperty("user.home") + FORM_LAYOUT_PROTOTYPES_FILENAME).get(formTitle.replace(" ", "").replace(":", "") );
				layoutProperties = readProperties(System.getProperty("user.home") + FORM_LAYOUT_PROTOTYPE_PROPERTIES_FILENAME);
				
				String submitFontColorCode = layoutProperties.getProperty(formLayoutPrototype+".submitFontColor", Integer.toHexString(submitFontColor.getRGB()).substring(2));
				submitFontColor = new Color(Integer.decode("0x"+submitFontColorCode));

				String labelFontColorCode = layoutProperties.getProperty(formLayoutPrototype+".labelFontColor", Integer.toHexString(labelFontColor.getRGB()).substring(2));
				labelFontColor = new Color(Integer.decode("0x"+labelFontColorCode));
				
				String sliderFontColorCode = layoutProperties.getProperty(formLayoutPrototype+".sliderFontColor", Integer.toHexString(sliderFontColor.getRGB()).substring(2));
				sliderFontColor = new Color(Integer.decode("0x"+sliderFontColorCode));
				
				String textAreaFontColorCode = layoutProperties.getProperty(formLayoutPrototype+".textAreaFontColor", Integer.toHexString(textAreaFontColor.getRGB()).substring(2));
				textAreaFontColor = new Color(Integer.decode("0x"+textAreaFontColorCode));

				String editFontColorCode = layoutProperties.getProperty(formLayoutPrototype+".editFontColor", Integer.toHexString(editFontColor.getRGB()).substring(2));
				editFontColor = new Color(Integer.decode("0x"+editFontColorCode));
				
				String selectFontColorCode = layoutProperties.getProperty(formLayoutPrototype+".selectFontColor", Integer.toHexString(selectFontColor.getRGB()).substring(2));
				selectFontColor = new Color(Integer.decode("0x"+selectFontColorCode));
				
				String selectBackgroundColorCode = layoutProperties.getProperty(formLayoutPrototype+".selectBackgroundColor", Integer.toHexString(selectBackgroundColor.getRGB()).substring(2));
				selectBackgroundColor = new Color(Integer.decode("0x"+selectBackgroundColorCode));
								
				String groupLabelFontColorCode = layoutProperties.getProperty(formLayoutPrototype+".groupLabelFontColor", Integer.toHexString(groupLabelFontColor.getRGB()).substring(2));
				groupLabelFontColor = new Color(Integer.decode("0x" + groupLabelFontColorCode));
				
				String wrapValue = layoutProperties.getProperty(formLayoutPrototype+".wrapSubmitText", "");			
				if (!wrapValue.isEmpty()) wrapSubmitText = wrapValue.equals("yes");
				
				
				String stretchValue = layoutProperties.getProperty(formLayoutPrototype+".stretchSelectToMax", "");			
				if (!stretchValue.isEmpty()) stretchSelectToMax = stretchValue.equals("yes");
				
				selectVisibleRows = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".selectVisibleRows", Integer.valueOf(selectVisibleRows).toString())).intValue();			
				
				wrapSubmitIncreaseWidth = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".wrapSubmitIncreaseWidth", Integer.valueOf(wrapSubmitIncreaseWidth).toString())).intValue();
				wrapSubmitIncreaseHeight = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".wrapSubmitIncreaseHeight", Integer.valueOf(wrapSubmitIncreaseHeight).toString())).intValue();
				
				submitIcon = layoutProperties.getProperty(formLayoutPrototype+".submitIcon", submitIcon);
				submitFont = layoutProperties.getProperty(formLayoutPrototype+".submitFont", submitFont);
				submitFontStyle = layoutProperties.getProperty(formLayoutPrototype+".submitFontStyle", submitFontStyle);
				submitFontSize = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".submitFontSize", Integer.valueOf(submitFontSize).toString())).intValue();
				
				textAreaFont = layoutProperties.getProperty(formLayoutPrototype+".textAreaFont", textAreaFont);
				textAreaFontStyle = layoutProperties.getProperty(formLayoutPrototype+".textAreaFontStyle", textAreaFontStyle);
				textAreaFontSize = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".textAreaFontSize", Integer.valueOf(textAreaFontSize).toString())).intValue();

				sliderFont = layoutProperties.getProperty(formLayoutPrototype+".sliderFont", sliderFont);
				sliderFontStyle = layoutProperties.getProperty(formLayoutPrototype+".sliderFontStyle", sliderFontStyle);
				sliderFontSize = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".sliderFontSize", Integer.valueOf(sliderFontSize).toString())).intValue();

				selectFont = layoutProperties.getProperty(formLayoutPrototype+".selectFont", selectFont);
				selectFontStyle = layoutProperties.getProperty(formLayoutPrototype+".selectFontStyle", selectFontStyle);
				selectFontSize = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".selectFontSize", Integer.valueOf(selectFontSize).toString())).intValue();
				
				String backgroundColorCode = layoutProperties.getProperty(formLayoutPrototype+".defaultBackgroundColor", Integer.toHexString(backgroundColor.getRGB()).substring(2));
				backgroundColor = new Color(Integer.decode("0x"+backgroundColorCode));
				screenBorder = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".screenBorder", Integer.valueOf(screenBorder).toString())).intValue();
				componentsBorder = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".componentsBorder", Integer.valueOf(componentsBorder).toString())).intValue();
				keepWideLastControlsColumn = new Boolean(layoutProperties.getProperty(formLayoutPrototype+".keepWideLastControlsColumn",new Boolean(keepWideLastControlsColumn).toString())).booleanValue();
				
				String foregroundTitleColorCode = layoutProperties.getProperty(formLayoutPrototype+".foregroundTitleColor", Integer.toHexString(foregroundTitleColor.getRGB()).substring(2));
				foregroundTitleColor = new Color(Integer.decode("0x"+foregroundTitleColorCode));
				
				titleFont = layoutProperties.getProperty(formLayoutPrototype+".titleFont", titleFont);
				titleFontStyle = layoutProperties.getProperty(formLayoutPrototype+".titleFontStyle", titleFontStyle);
				titleFontSize = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".titleFontSize", Integer.valueOf(titleFontSize).toString())).intValue();

				personaLogoImage = layoutProperties.getProperty(formLayoutPrototype+".personaLogoImage", personaLogoImage);
				personaTitleImage = layoutProperties.getProperty(formLayoutPrototype+".personaTitleImage", personaTitleImage);
				personaSoundImage = layoutProperties.getProperty(formLayoutPrototype+".personaSoundImage", personaSoundImage);
				
				defaultMenuCols = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".defaultMenuCols", Integer.valueOf(defaultMenuCols).toString())).intValue();
				defaultCols = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".columns", Integer.valueOf(defaultCols).toString())).intValue();
				defaultRows = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".rows", Integer.valueOf(defaultRows).toString())).intValue();
							
				labelFont = layoutProperties.getProperty(formLayoutPrototype+".labelFont", labelFont);
				labelFontStyle = layoutProperties.getProperty(formLayoutPrototype+".labelFontStyle", labelFontStyle);
				labelFontSize = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".labelFontSize", Integer.valueOf(labelFontSize).toString())).intValue();
				
				editFont = layoutProperties.getProperty(formLayoutPrototype+".editFont", editFont);
				editFontStyle = layoutProperties.getProperty(formLayoutPrototype+".editFontStyle", editFontStyle);
				editFontSizeEditable = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".editFontSizeEditable", Integer.valueOf(editFontSizeEditable).toString())).intValue();
				editFontSizeReadOnly = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".editFontSizeReadOnly", Integer.valueOf(editFontSizeReadOnly).toString())).intValue();
				
				groupLabelFont = layoutProperties.getProperty(formLayoutPrototype+".groupLabelFont", groupLabelFont);
				groupLabelFontStyle = layoutProperties.getProperty(formLayoutPrototype+".groupLabelFontStyle", groupLabelFontStyle);
				groupLabelFontSize = Integer.valueOf(layoutProperties.getProperty(formLayoutPrototype+".groupLabelFontSize", Integer.valueOf(groupLabelFontSize).toString())).intValue();
				
				defaultMenuHAlign = layoutProperties.getProperty(formLayoutPrototype+".MenuHAlign", defaultMenuHAlign);
				defaultMenuVAlign = layoutProperties.getProperty(formLayoutPrototype+".MenuVAlign", defaultMenuVAlign);
				
				defaultVAlign = layoutProperties.getProperty(formLayoutPrototype+".VAlign", defaultVAlign);
				defaultHAlign = layoutProperties.getProperty(formLayoutPrototype+".HAlign", defaultHAlign);
				
				String messagesFile = layoutProperties.getProperty("General.GuiMessages", GUI_MESSAGES_FILENAME); 
				Properties messageProperties = readProperties(System.getProperty("user.home") + messagesFile);
				if (messageProperties==null)
					messageProperties = new Properties();
				searchBoxLabel = messageProperties.getProperty(formLayoutPrototype+".searchBoxLabel", searchBoxLabel);
				soundLabel = messageProperties.getProperty(formLayoutPrototype+".soundLabel", soundLabel);
				dateTimeLabel = messageProperties.getProperty(formLayoutPrototype+".dateTimeLabel", dateTimeLabel);
	...
	public String getSpecialProperty(String propertySuffix, String defaultValue){
		try
		{
			if ((this.formLayoutPrototype != null) && (propertySuffix != null))
				return layoutProperties.getProperty(this.formLayoutPrototype + "." + propertySuffix, defaultValue);
			else
				return defaultValue;
		}
	    catch (Exception e){
