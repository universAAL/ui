 ...
 *
 */
public class RepeatTablePane extends JPanel implements ActionListener, ListSelectionListener {
	public static final long serialVersionUID = RepeatTablePane.class.hashCode();
		...
		for (int i=0; i<selectionControls.length; i++)
			for (int j=0; j<siblings.length; j++)
				if (siblings[j] instanceof JComponent
						&&  ((JComponent) siblings[j]).getClientProperty(
								SwingRenderer.PERSONA_FORM_CONTROL) == selectionControls[i]) {
