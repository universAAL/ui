...
//import org.persona.platform.profiling.ontology.service.
...
public class Login extends JFrame implements ActionListener {
	...
	/**
	 * Login Constructor.
	 *  Will build login window for user to input login data
	...
	/**
	 * Show Main Menu GUI.
	 * It will generate an input event 
	 * @param user User login data 
	 */
	private void showMainGUI(User user){
		Activator.user = user;
		ip.publish(new InputEvent(user, null, InputEvent.uAAL_MAIN_MENU_REQUEST));	  
	}

	/**
	 * 	JFrame event Collector.
	 *  		will perform actions on buttons push
	 *  		Login: 	send login info through {@link Login#showMainGUI(User)}
	 *  		Cancel: Stop Application
	 *  
	 *   TODO: true user identification using profile database!
	 *   TODO: Cancel should not exit application, other action needed.
	 *   
	 */
	public void actionPerformed(ActionEvent ae)
	{
		...
		if(ae.getSource().equals(LoginButton)){
			...
			// TODO: user identification (login-screen when users are in profile database!)
			// For now: just initialize the test user
			//User user = allowedCredentials(username, password);
			ElderlyUser user =  new ElderlyUser(Constants.uAAL_MIDDLEWARE_LOCAL_ID_PREFIX + "saied");
		...
		if (outputs == null)
			...
			for (Iterator i = outputs.iterator(); i.hasNext();) {
				...
				if (output.getURI().equals(expectedOutput))
					...
					log.error("GUIIOHandler - output ignored: {}",output.getURI());
			}
		return returnValue;
	}
