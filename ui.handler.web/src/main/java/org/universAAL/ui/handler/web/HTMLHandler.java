...
public class HTMLHandler extends GatewayPort implements IWebHandler{
	...
	public static final String PERSONA_ASSOCIATED_LABEL = "urn:org.persona.dialog:AssociatedLabel";
	public static final String PERSONA_CLOCK_THREAD = "urn:org.persona.dialog:TheClockThread";
	public static final String PERSONA_FORM_CONTROL = "urn:org.persona.dialog:FormControl";
	public static final String PERSONA_PANEL_COLUMNS = "urn:org.persona.dialog:PanelColumns";
	...
	private final static Logger log=LoggerFactory.getLogger(HTMLHandler.class);
...
//								new PhysicalImpairment(LevelRating.low)})));
	...
	private String renderGroupControl(Group ctrl, Hashtable<String,FormControl> assoc, boolean repeat) {
		...
		for (int i = 0; i < children.length; i++) {
			if(repeat)html.append("<td>");
			if (children[i] instanceof InputField) {
				html.append(renderInputControl((InputField) children[i],assoc,repeat));
			} else if (children[i] instanceof SimpleOutput) {
				html.append(renderOutputControl((SimpleOutput) children[i],repeat));
			} else if (children[i] instanceof Select1) {
				html.append(renderSelect1Control((Select1) children[i],assoc,repeat));
			} else if (children[i] instanceof Select) {
				html.append(renderSelectControl((Select) children[i],assoc,repeat));
			} else if (children[i] instanceof Repeat) {
				html.append("<fieldset>");
				if(((Group)children[i]).getLabel()!=null){
					html.append("<legend>"+((Group)children[i]).getLabel()+"</legend>");
				}
				...
				html.append(renderMediaObject((MediaObject)children[i],repeat));
			} else if (children[i] instanceof TextArea) {
				html.append(renderTextArea((TextArea)children[i],assoc,repeat));
			} else if (children[i] instanceof Range) {
				Range tempRng=(Range)children[i];
				if((Integer)tempRng.getMaxValue()>10)
					html.append(renderSpinnerControl((Range)children[i],assoc,repeat));
				else
					html.append(renderRangeControl((Range)children[i],assoc,repeat));
			}
			html.append(System.getProperty("line.separator")+(repeat?"</td>":"<br>"));
		}
		return html.toString();
	}

	private String renderGroupControl(Group group, Hashtable<String,FormControl> assoc) {
	...
	private String renderOutputControl(final FormControl ctrl, boolean mute) {
		...
		StringBuilder html = new StringBuilder();
		if(cl!=null && !mute)if(cl.getText()!=null)html.append("<b>"+cl.getText()+" </b>");
	...
	private String renderInputControl(final FormControl ctrl, Hashtable<String,FormControl> assoc, boolean mute) {
		StringBuilder html = new StringBuilder();
		Label cl = ctrl.getLabel();

		Object initVal = ctrl.getValue();

		if(cl!=null && !mute)if(cl.getText()!=null)html.append("<label><b>"+cl.getText()+" </b>");
		if (ctrl.isOfBooleanType()&&initVal instanceof Boolean) {		
			html.append("<input type=\"checkbox\" name=\"" +ctrl.getURI()+
					"\" value=\""+((Boolean) initVal).booleanValue()+
					"\" "+((((Boolean) initVal).booleanValue())?" checked=\"checked\"":"\"\"")+" />");
		} else {
			String type="text";
			if(((InputField)ctrl).isSecret())type="password";
			html.append("<input type=\""+type+"\" name=\"" +ctrl.getURI()+
					"\" value=\""+((initVal!=null)?initVal.toString():"")+
					"\" size=\" "+((initVal!=null)?initVal.toString().length():"")+"\" />");
		}
		if(cl!=null && !mute)if(cl.getText()!=null)html.append("</label>");
		assoc.put(ctrl.getURI(), ctrl);
		return html.toString();
	}

	private Object renderSelect1Control(Select1 ctrl, Hashtable<String,FormControl> assoc, boolean mute) {
		Label cl = ctrl.getLabel();
		StringBuilder html = new StringBuilder();
		if(cl!=null && !mute)if(cl.getText()!=null)html.append("<label><b>"+cl.getText()+" </b>");
	...
	private Object renderSelectControl(Select ctrl, Hashtable<String,FormControl> assoc, boolean mute) {
		Label cl = ctrl.getLabel();
		StringBuilder html = new StringBuilder();
		if(cl!=null && !mute)if(cl.getText()!=null)html.append("<label><b>"+cl.getText()+" </b>");
		html.append("<select name=\"" +ctrl.getURI()+"\" size=\"4\" multiple=\"multiple\">");
		html.append(System.getProperty("line.separator"));
		Label[] labels=ctrl.getChoices();
		for(int i=0;i<labels.length;i++){
			html.append("<option>");
	...
	private Object renderSubmitControl(Submit ctrl, Hashtable<String,FormControl> assoc) {
		Label cl = ctrl.getLabel();
		StringBuilder html = new StringBuilder();
		String imageURL=null;
		if (cl!=null)imageURL=cl.getIconURL();
		if(imageURL!=null){
//			html.append("<input type=\"image\" src=\""+imageURL+"\" ");//TODO doesnt work like submit
			html.append("<input type=\"submit\" ");
		}else{
			html.append("<input type=\"submit\" ");
		}
		html.append(" name=\"submit_"+ctrl.getID()+
				"\" value=\""+(cl!=null?(cl.getText()!=null?cl.getText():ctrl.getID()):(ctrl.getID()))+
				"\" />");
		assoc.put(ctrl.getID(), ctrl);
		return html.toString();
	}
	
	private Object renderRepeat(Repeat ctrl, Hashtable<String,FormControl> assoc) {
		StringBuilder html = new StringBuilder();
		...
		for(int i=0;i<ctrl.getNumberOfValues();i++){
			html.append("<tr>");
			ctrl.setSelection(i);
			html.append(renderGroupControl((groupflag?(Group)elems[0]:ctrl),assoc,true));
			assoc.put(ctrl.getURI(), ctrl);
			html.append("</tr>");
		}
	...
	private String renderMediaObject(MediaObject ctrl, boolean mute) {
		...
		if(src!=null){
			...
			if(w>0 && h>0)html.append(" width=\""+w+"\" height=\""+h+"\" ");
			html.append(" />");
		}
		return html.toString();
	}
	
	private String renderTextArea(TextArea ctrl, Hashtable<String,FormControl> assoc, boolean mute) {
		Label cl = ctrl.getLabel();
		StringBuilder html = new StringBuilder();
		if(cl!=null && !mute)if(cl.getText()!=null)html.append("<label><b>"+cl.getText()+" </b>");
		String initVal = (String)ctrl.getValue();
		html.append("<textarea name=\""+ctrl.getURI()+"\" rows=\"4\" cols=\"20\">");
		if(initVal!=null){
	...
	private String renderRangeControl(Range ctrl, Hashtable<String,FormControl> assoc, boolean mute) {
		// TODO Specific control of range (slider)
		return renderSpinnerControl(ctrl,assoc,mute);
	}

	private String renderSpinnerControl(Range ctrl, Hashtable<String,FormControl> assoc, boolean mute) {
		...
		html.append("<select name=\"" +ctrl.getURI()+"\" tabindex=\""+(val-min)+"\">");
		html.append(System.getProperty("line.separator"));
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(!userSessions.containsKey(userURI)){
			...
			o=publish(event,Boolean.TRUE);//Sends the INPUT ev and wait for OUTPUT ev
			...
			for(;names.hasMoreElements();){
				...
				if(name.startsWith("submit_")){
					...
					selectedSubmit=(Submit)ses.getCurrentFormAssociation().get(submit);
				}else{
					FormControl ctrl=(FormControl) ses.getCurrentFormAssociation().get(name);
					if(ctrl instanceof InputField){
						if(ctrl.isOfBooleanType()){
							String[] values=req.getParameterValues(name);
							Boolean value=(values.length>1)?true:Boolean.parseBoolean(values[0]);
							((InputField)ctrl).storeUserInput(value);
						}else{
							((InputField)ctrl).storeUserInput(req.getParameter(name));
						}
					}else if(ctrl instanceof Select){
						if(ctrl instanceof Select1){
							if(ctrl.isOfBooleanType()){
								((Select1)ctrl).storeUserInput(Boolean.parseBoolean(req.getParameter(name)));
							}else{
								((Select1)ctrl).storeUserInputByLabelString(req.getParameter(name));
							}
						}else{
							if(ctrl.isOfBooleanType()){
								String[] values= req.getParameterValues(name);
						...
						((TextArea)ctrl).storeUserInput(req.getParameter(name));
					}else if(ctrl instanceof Range){
						((Range)ctrl).storeUserInput(Integer.parseInt(req.getParameter(name)));
					}
				}
			}
			//build and send the InputEvent
			o=dialogFinished(selectedSubmit,userURI);//Sends the INPUT ev and wait for OUTPUT ev
			ses.setCurrentOutputEvent(o);
		}
		...
		BufferedReader reader = new BufferedReader(new InputStreamReader(pageReader));
		StringBuilder html = new StringBuilder();
		String line;
		//Build the html
		Hashtable<String,FormControl> assoc=new Hashtable<String,FormControl>();//This hashtable will replace the old one containing the past input
...
//				}
	...
	public OutputEvent dialogFinished(Submit s, String userURI) {
		...
		// for the next line, see the comment within handleOutputEvent() above [preserved from SwingRenderer]
		...
		if (o instanceof OutputEvent) {
			...
			return this.publish(new InputEvent(
					((OutputEvent) o).getAddressedUser(),
 ((OutputEvent) o)
					.getPresentationAbsLocation(),
					s),Boolean.FALSE);
		} else {
			os.dialogFinished(s, false);
			synchronized (os) {
				InputEvent ie = new InputEvent(
						((WebIOSession)this.userSessions.get(userURI)).getCurrentOutputEvent().getAddressedUser(),
						((WebIOSession)this.userSessions.get(userURI)).getCurrentOutputEvent().getPresentationAbsLocation(),
						s);
				if (s.getDialogID().equals(os.dialogID))
					((WebIOSession)this.userSessions.get(userURI)).setCurrentOutputEvent(null);
