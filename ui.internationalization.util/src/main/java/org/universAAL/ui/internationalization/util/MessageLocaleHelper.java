 ...
 * if preferred not available) language.
 * Determines the {@link Locale} of the preferred (or secondary) Language for
 * the user stored in {@link UIPreferencesSubProfile}.
 * Then it can be used as normal messages class, to retrieve the correct
 * internationalized messages for the given user.
 * 
 * @author amedrano
 * @author eandgrg
 */
public class MessageLocaleHelper {
	...
	private static final String OUTPUT_GETSUBPROFILES = UIPreferencesProfileOntology.NAMESPACE
			+ "outputSubprofile";
	...
	/**
	 * used to log, and call the service to retrieve the {@link UIPreferencesSubProfile}.
	 */
	 ...
	 * </code></pre>
	 * 
	 * @param mc The {@link ModuleContext} to enable logging and service call.
	 * @param user The {@link User} to address the messages.
	 * @param urlList the list of alternative {@link URL} locations for messages.
	 * @throws Exception 
	 * @see {@link MessageLocaleHelper#reloadMessages()}
	 */
	public MessageLocaleHelper(ModuleContext mc, User user, List urlList) throws Exception {
	...
	/**
	 * This constructor is to be used when the {@link UIPreferencesSubProfile} is already available,
	 * IE: by the DialogManager.
	 * @param mc
	...
	public MessageLocaleHelper(ModuleContext mc,
			UIPreferencesSubProfile uiPreferencesSubprofile, List urlList) throws Exception {
	...
	private UIPreferencesSubProfile getUIPreferencesSubProfileForUser(User user) {
		...
		if (resp.getCallStatus() == CallStatus.succeeded) {
			try {
				...
				LogUtils.logError(context, this.getClass(),
			...
			LogUtils.logWarn(
					...
					this.getClass(),
					"getUIPreferencesSubProfileForUser",
					new Object[] { "returned: " + resp.getCallStatus().name() },
					null);
			return null;
		}
	}
	 ...
	 * Used when messages files are updated.
	 * It checks all the Alternative Locations for the preferred language, 
	 * the first instance (in the alternatives list) of preferred language 
	 * locale messages is loaded. If there are no preferred language locale 
	 * messages files, then the first instance of secondary language is loaded.
	 * As last resort if nor primary nor secondary language is found then
	 * the first instance of default message file is loaded.
	 * It will throw an exception if no files could be loaded.
	 * @throws Exception 
	 */
	 ...
	 * @throws IOException
	 */
	private int tryToLoadMessagesForm(URL url)
