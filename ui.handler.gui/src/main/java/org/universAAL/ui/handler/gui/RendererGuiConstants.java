...
public class RendererGuiConstants {
	...
	private static final String SERVICE_ICON_MAPPER_FILENAME = "serviceIconMapper.properties";
	...
	private void initializeVars() {
		// TODO: externalize initialization constants, for customization
		// purposes.
	...
	private Properties readProperties(String propertiesFileName) {
		try {
			 ...
			 * (propertiesFile.exists()) { Properties properties = new
			 * Properties(); FileInputStream inputStream = new FileInputStream(
			 * propertiesFile); properties.load(inputStream); return properties;
			 * } else {
	...
	/**
	 * Fetches url of the service icon for the main screen.
	 * 
	 * @param serviceName
	 *            service name, also 1st parameter in dm config file main_menu
	 * @return url of the icon that belongs to the service
	 */
	public String fetchServiceIconURL(String serviceName) {
		return readProperties(SERVICE_ICON_MAPPER_FILENAME).getProperty(
				serviceName);
	}
		...
		try {
			if (readProperties(FORM_LAYOUT_PROTOTYPES_FILENAME).containsKey(
				...
				this.formLayoutPrototype = (String) readProperties(
						...
						formTitle.replace(" ", "").replace(":", ""));
				layoutProperties = readProperties(FORM_LAYOUT_PROTOTYPE_PROPERTIES_FILENAME);
