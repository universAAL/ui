 ...
 * component divides the screen into 4 panes (header,footer,border, and main).
 * Implements: {@link ActionListener} to receive action events.
 * {@link FocusListener} to receive keyboard events on focus. {@link Scrollable}
 * to scroll panes. {@link ChangeListener} to listen to change events.
 * {@link ListSelectionListener} to listen to selection events on lists.
 * Extends: {@link JFrame} being the main frame container.
 */
public class SwingRenderer extends JFrame implements ActionListener,
		FocusListener, Scrollable, ChangeListener, ListSelectionListener {
	 ...
	 * @param context
	 */
	public SwingRenderer(GUIIOHandler theHandler) throws HeadlessException {
		super();
	...
	private JLabel addControlLabel(FormControl ctrl) {
		...
		String htmlvalue = (labelStr == null) ? "" : "<html>" + "<FONT COLOR=#"
				+ rendererGuiConstants.getLabelFontColor() + ">" + cl
				+ "</FONT>";
		JLabel label = new JLabel(htmlvalue);
		if (iconURL != null) {
			labelStr = ctrl.getHintString();
			label.setIcon((labelStr == null) ? new ImageIcon(iconURL)
					: new ImageIcon(iconURL, labelStr));
		}
		label.setMinimumSize(label.getPreferredSize());
		setLookAndFeelFont(rendererGuiConstants.getLabelFont(),
				rendererGuiConstants.getLabelFontStyle(), rendererGuiConstants
						.getLabelFontSize(), label);
	...
	private synchronized void checkInput(JComponent src) {
		...
		if (o instanceof InputField) {
			...
			ChoiceItem choiceItem = (ChoiceItem) ((Select1) o).getChoices()[((JComboBox) src)
					.getSelectedIndex()];
	...
	private void createBorderLabel(JPanel pane, Group g) {
		...
		if (lt != null) {
			// lt = lt + " - " + pane.getName() + " - " +
			// pane.getLayout().getClass().toString();
			TitledBorder aux = BorderFactory.createTitledBorder(lt);
			aux.setTitleFont(getLookAndFeelFont(rendererGuiConstants
					.getGroupLabelFont(), rendererGuiConstants
					.getGroupLabelFontStyle(), rendererGuiConstants
	...
	private JComponent createDateLabel() {
		...
		ImageIcon di = getIcon(rendererGuiConstants.getPersonaSoundImage(),
				null);
	...
	private Container createGUIFooter(Group buttons) {
		...
		southPane.setBackground(rendererGuiConstants
		...
		// southPane.putClientProperty(PERSONA_CLOCK_THREAD, new
		// DateTime(date).runTheClock());
		...
		if (buttons != null) {
			...
			if (children != null && children.length > 0) {
				...
				pane.setBackground(rendererGuiConstants
						.getDefaultBackgroundColor());
				...
				for (int i = 0; i < children.length; i++)
					if (children[i] instanceof Submit) {
						JComponent child = renderSubmitControl(
								(Submit) children[i], false);
						if (child != null)
							pane.add(child);
					}
	...
	private Container createGUIHeader(String title, boolean isSystemMenu) {
		...
		TableLayout headerLayout = new TableLayout(
				isSystemMenu ? sizeForMainMenu : sizeforApps);
		headerPane.setLayout(headerLayout);

		headerPane.setBackground(rendererGuiConstants
				.getDefaultBackgroundColor());
		...
		titleLabel
				.setForeground(rendererGuiConstants.getForegroundTitleColor());
		setLookAndFeelFont(rendererGuiConstants.getTitleFont(),
				rendererGuiConstants.getTitleFontStyle(), rendererGuiConstants
						.getTitleFontSize(), titleLabel);

		if (isSystemMenu) {
			titleLabel.setText((null == icon) ? "universAAL" : "");
	...
	private Container createSubmits(Group submits) {
		...
		submitsPanel.setBackground(rendererGuiConstants
				.getDefaultBackgroundColor());
		...
		// submitsPanel.putClientProperty(PERSONA_CLOCK_THREAD, new
		// DateTime(date).runTheClock()); //org.persona.dilog or
		// org.persona.dialog?
		...
		if (children != null)
			for (int i = 0; i < children.length; i++)
				if (children[i] instanceof JPanel) {
					Object o = ((JPanel) children[i])
							.getClientProperty(PERSONA_CLOCK_THREAD);
	...
	/**
	 * Sets the initial contents of our {@link JFrame} {@link Container}
	 */
	public void initDefaultContainer(Form f) {
		...
		double preferred = TableLayout.PREFERRED;
		double fill = TableLayout.FILL;
		double size[][] = { { border, fill, preferred, border },
				{ border, preferred, fill, preferred, border } };
		TableLayout layout = new TableLayout(size);
		content.setLayout(layout);
		content.add(createGUIHeader(f.getTitle(), f.isSystemMenu()),
				"0, 1, 1, 1");
	}
	...
	public void renderForm(Form f) {
		...
		mainPanelData.setBackground(rendererGuiConstants
				.getDefaultBackgroundColor());
		if (f.isSystemMenu()) {
			content.add(createGUIFooter(f.getStandardButtons()), "1, 3, 2, 3");
			mainPanelData = renderGroupControl(f.getIOControls(),
					rendererGuiConstants.getDefaultMenuCols(), true);
		} else {
		...
		if (mainPanelData != null) {
			if (f.isSystemMenu())
				mainPanelData.setName(rendererGuiConstants
						.getMainMenuPanelName()); // setting name before adding
			// the panel to scrollPane,
			// will result in deleting
			// the panel name by the
			// scrollPane
			else
				mainPanelData.setName(rendererGuiConstants
						.getMainPanelPanelName()); // setting name before adding
			// the panel to scrollPane,
			// will result in deleting
			// the panel name by the
			// scrollPane
	...
	public void repaintIcons(Container c) {
		for (int i = 0; i < c.getComponentCount(); i++) {
			try {
				boolean canBeSetIcon = false; // the Component contains setIcon,
				// getIcon methods (JButton,
				// JLabel etc)
				int stretched = 0;
				...
				if (c.getComponent(i) instanceof JComponent) {
					...
					for (int mIdx = 0; mIdx < methList.length; mIdx++) {
						canBeSetIcon = (canBeSetIcon || (methList[mIdx]
								.getName() == "getIcon"));
					}
					Object stretchedObj = component
							.getClientProperty("Stretched");
					stretched = (null == stretchedObj) ? 0
							: ((Integer) stretchedObj).intValue();
				}

				if ((canBeSetIcon) && (stretched == 1)) {
					if (c.getComponent(i) instanceof JButton) {
						ImageIcon icn = (ImageIcon) ((JButton) c
								.getComponent(i)).getIcon();
						Object stretchedObj = ((JComponent) c.getComponent(i))
								.getClientProperty("verticallyStretchedToText");
						int stretchedButtonToText = (null == stretchedObj) ? 0
								: ((Integer) stretchedObj).intValue();

						if (null != icn) {
							if ((((JButton) c.getComponent(i)).getText() != null)
									&& (!((JButton) c.getComponent(i))
											.getText().isEmpty())
									&& (stretchedButtonToText == 1)) {
								((JButton) c.getComponent(i))
										.setIcon(scaleIcon(
												icn.getImage(),
												(int) ((JButton) c
								...
								((JButton) c.getComponent(i))
										.setIcon(scaleIcon(icn.getImage(),
												(int) ((JButton) c
														.getComponent(i))
														.getWidth(),
												(int) ((JButton) c
					...
					} else if (c.getComponent(i) instanceof JLabel) {
						ImageIcon icn = (ImageIcon) ((JLabel) c.getComponent(i))
								.getIcon();
						if (null != icn)
							((JLabel) c.getComponent(i)).setIcon(scaleIcon(icn
									.getImage(), (int) ((JLabel) c
									.getComponent(i)).getWidth(),
									(int) ((JLabel) c.getComponent(i))
											.getHeight()));
					}
	...
	private JPanel renderGroupControl(Group ctrl, int maxCols,
			boolean isMainMenu) {
		...
		System.err.println(ctrl.getFormObject().getTitle() + "\n"
		...
		for (int i = 0; i < children.length; i++) {
			...
			if (children[i] instanceof InputField) {
				...
				if (repeats == null || repeats.length != 1)
					...
					// use the dummy group for rendering the children, but
					// overwrite it by the two changes below
					child = renderGroupControl((Group) repeats[0], maxCol,
							false);
					// change the border
					createBorderLabel((JPanel) child, (Repeat) children[i]);
					// change the associated ctrl
					((JPanel) child).putClientProperty(PERSONA_FORM_CONTROL,
							children[i]);
				} else
					child = renderGroupControl((Group) children[i], maxCol,
							false);
				newLine = true;
				...
				child = renderSubmitControl((Submit) children[i], isMainMenu);
				...
				// photographLabel.putClientProperty("Stretched", new
				// Integer(1));
			...
			Object[] added = new Object[] {
					child.getClientProperty(PERSONA_ASSOCIATED_LABEL), child };
			for (int j = 0; j < 2; j++)
				if (j == 1 || added[0] instanceof JLabel) {
					...
					if (added[j] instanceof JPanel) {
						Object o = ((JPanel) added[j])
								.getClientProperty(PERSONA_PANEL_COLUMNS);
		...
		Repeat r = (ctrl instanceof Repeat) ? (Repeat) ctrl : (ctrl
				.getParentGroup() instanceof Repeat) ? (Repeat) ctrl
				.getParentGroup() : null;
		if (r != null) {
			GridBagConstraints gbc = new GridBagConstraints();
			gbc.gridx = 0;
	...
	private JSlider renderRangeControl(Range rng) {
		...
		JSlider rangeComp = new JSlider(JSlider.HORIZONTAL, mnValue, mxValue,
				initValue);
		...
		rangeComp.setBorder(BorderFactory.createEmptyBorder(0, 0, 10, 0));
		setLookAndFeelFont(rendererGuiConstants.getSliderFont(),
				rendererGuiConstants.getSliderFontStyle(), rendererGuiConstants
						.getSliderFontSize(), rangeComp);
		return rangeComp;
	}
	...
	private JTextPane createTextPane(String initValue) {
		String[] initStyles = { "regular", "format" };
		...
		try {
			if (!myStyle) {
				doc.insertString(doc.getLength(), initValue, doc
	...
	protected boolean addStylesToDocument(StyledDocument doc) {
		...
		StyleConstants.setFontFamily(def, rendererGuiConstants
				.getTextAreaFont());
		StyleConstants.setFontSize(def, rendererGuiConstants
				.getTextAreaFontSize());
		...
		if (fontStyle.equalsIgnoreCase("italic")) {
			...
			StyleConstants.setFontSize(s, rendererGuiConstants
					.getTextAreaFontSize());
			formatSet = true;
		...
		try {
			if ((mo != null) && (mo.getContentURL() != null))
				if ((mo.getResolutionPreferredX() > 0)
						&& (mo.getResolutionPreferredY() > 0))
					return scaleIcon(getIcon(mo.getContentURL(), null)
							.getImage(), mo.getResolutionPreferredX(), mo
							.getResolutionPreferredY());
				else
					return getIcon(mo.getContentURL(), null);
			else
				System.err
						.println("Media object or mediaObject.ContentURL empty!");
			return null;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	private JComponent renderInputControl(final FormControl ctrl,
			boolean isEditable) {
		...
		if (ctrl.isOfBooleanType()) {
			...
			if (isEditable) {
				...
				if (initVal != null)
					tf.setSize(fm.stringWidth(initVal.toString())
							/ tf.getFont().getSize(), tf.getHeight());
				else
				...
				((JTextArea) tf).setLineWrap(false); // expand the textarea to
				// the longest string
				// (use \n for new line)
			}
		...
		setLookAndFeelFont(rendererGuiConstants.getEditFont(),
				rendererGuiConstants.getEditFontStyle(),
				(isEditable ? rendererGuiConstants.getEditFontSizeEditable()
						: rendererGuiConstants.getEditFontSizeReadOnly()), res);
	...
	private JComponent renderSelect1Control(Select1 ctrl) {
		...
		if (ctrl.isMultilevel()) {
			...
			for (int i = 0; i < ctrl.getChoices().length; i++) {
				if (((ChoiceItem) ctrl.getChoices()[i]).getValue().equals(
						ctrl.getValue())) {
		...
		setLookAndFeelFont(rendererGuiConstants.getSelectFont(),
				rendererGuiConstants.getSelectFontStyle(), rendererGuiConstants
						.getSelectFontSize(), res);
	...
	private JButton renderSubmitControl(Submit ctrl, boolean isMainMenu) {
		...
		String htmlvalue = isEmptyString(labelStr) ? "" : "<html>"
				...
				+ Integer.toHexString(
						rendererGuiConstants.getSubmitFontColor().getRGB())
						.substring(2) + ">" + ctrl.getLabel() + "</FONT>";

		if (rendererGuiConstants.getWrapSubmitText())
			htmlvalue = htmlvalue.replaceFirst("FONT COLOR", "FONT_COLOR")
					.replace(" ", "<P>").replaceFirst("FONT_COLOR",
							"FONT COLOR");
		// e.g. htmlvalue= <html><FONT COLOR=#11aa7f>Agenda</FONT>
		JButton submit = new JButton(htmlvalue);

		if (iconURL != null) {
			labelStr = ctrl.getHintString();
			submit.setIcon((labelStr == null) ? getIcon(iconURL, null)
					: getIcon(iconURL, labelStr));
		}
		submit.putClientProperty(PERSONA_FORM_CONTROL, ctrl);
		setLookAndFeelFont(rendererGuiConstants.getSubmitFont(),
				rendererGuiConstants.getSubmitFontStyle(), rendererGuiConstants
						.getSubmitFontSize(), submit);

		submit.setBorderPainted(false);
		submit.setFocusPainted(false);
		submit.setContentAreaFilled(false);
		// submit.setMargin(new Insets(10,20, 10, 20)); //don't do that to any
		// button because there is not need. look the submit section on
	...
	private ImageIcon scaleIcon(final Image image, final int width,
			final int height) {
	...
	private void redrawContainer(Container container, int containerLevel,
			String mainContainerName) {
		System.err.println("]] redrawing container " + container.getName()
				+ " - mainContainerName = " + mainContainerName + " level = "
				+ containerLevel);
		// containerLevel++;
		for (int i = 0; i < container.getComponentCount(); i++) {
			if ((container.getComponent(i).getClass().getName()
					.endsWith("Pane"))
					|| (container.getComponent(i).getClass().getName()
							.endsWith("Panel"))
					|| (container.getComponent(i).getClass().getName()
							.endsWith("Frame"))
					|| (container.getComponent(i).getClass().getName()
				...
				if ((container.getComponent(i).getName() != null)
						&& ((container.getComponent(i).getName()
								.equals(rendererGuiConstants
										.getFooterPanelName()))
								|| (container.getComponent(i).getName()
										.equals(rendererGuiConstants
												.getHeaderPanelName()))
								...
								|| (container.getComponent(i).getName()
										.equals(rendererGuiConstants
												.getMainMenuPanelName())) || (container
								.getComponent(i).getName()
								.equals(rendererGuiConstants
										.getMainPanelPanelName())))) {
					mainContainer = container.getComponent(i).getName();
					containerLevel = 0;
				}
				redrawContainer(((Container) container.getComponent(i)),
						containerLevel + 1, mainContainer);
			}
		}
		if (container instanceof JScrollPane)
			// redrawFinalContainer((Container)((JScrollPane)container).getViewport(),
			// containerLevel, mainContainerName);
			System.out.println("scrollpane");
		else if ((container instanceof JViewport)
				&& (container.getComponentCount() > 0)) {
			redrawComponent(-1, container, container.getComponent(0),
					containerLevel, 0, 0, 0, container.getName(),
					mainContainerName);
		} else if (mainContainerName.equals(rendererGuiConstants
				.getMainMenuPanelName()))
			redrawMainMenu(container, containerLevel, mainContainerName);
		else
			redrawFinalContainer(container, containerLevel, mainContainerName);
	}

	private double[][] getLayoutSize(int appropriatePanelsPerRow,
			int appropriatePanelsPerColumn) {
		...
		if (appropriatePanelsPerRow == 1) {
			...
			for (int col = 0; col < appropriatePanelsPerRow * 2; col++) {
				if (col == ((appropriatePanelsPerRow * 2) - 2)) // last column
					// of controls
					if (rendererGuiConstants.keepWideLastControlsColumn())
						columns[col] = fill;
					else
		...
		for (int row = 0; row < Double.valueOf(appropriatePanelsPerColumn)
				.intValue() * 2; row++) {
			rows[row] = preferred;
			rows[++row] = border;
		}
	...
	private void redrawComponent(double viewPortWidth, Container container,
			Component component, int containerLevel, int row, int col,
			int componentIndex, String containerName, String mainContainerName) {
		if (component instanceof JButton) {
			JButton button = (JButton) component;
			if ((button.getText() != null) && (!button.getText().isEmpty())) {
				String FontName = rendererGuiConstants.getSpecialProperty(
						mainContainerName + ".Level" + containerLevel
								+ ".submitFont", rendererGuiConstants
								.getSubmitFont());
				String FontStyle = rendererGuiConstants.getSpecialProperty(
						mainContainerName + ".Level" + containerLevel
								+ ".submitFontStyle", rendererGuiConstants
								.getSubmitFontStyle());
				int FontSize = Integer.valueOf(
						rendererGuiConstants.getSpecialProperty(
								mainContainerName + ".Level" + containerLevel
				...
				String oldFontColor = button.getText().substring(
						button.getText().indexOf("#") + 1,
						button.getText().indexOf("#") + 7);
				String submitFontColorStr = rendererGuiConstants
						.getSpecialProperty(mainContainerName + ".Level"
								+ containerLevel + ".submitFontColor", null);
				String newFontColor = (submitFontColorStr == null) ? Integer
						.toHexString(
								rendererGuiConstants.getSubmitFontColor()
										.getRGB()).substring(2)
						: submitFontColorStr;

				button.setText(button.getText().replace(oldFontColor,
						newFontColor));

				String wrapSubmitsText = rendererGuiConstants
						.getSpecialProperty(mainContainerName + ".Level"
								+ containerLevel + ".wrapSubmitText", "");
				if ((!wrapSubmitsText.isEmpty())
						&& (wrapSubmitsText.equalsIgnoreCase("no"))) {
					button.setText(button.getText().replace("<P>", " "));
					button.putClientProperty("verticallyStretchedToText",
							new Integer(1));
				}
			}
			component = button;
		} else if (component instanceof JTextField) {
			String FontName = rendererGuiConstants
					.getSpecialProperty(mainContainerName + ".Level"
							+ containerLevel + ".editFont",
							rendererGuiConstants.getEditFont());
			String FontStyle = rendererGuiConstants.getSpecialProperty(
					mainContainerName + ".Level" + containerLevel
							+ ".editFontStyle", rendererGuiConstants
							.getEditFontStyle());
			int fontSize = ((JTextField) component).isEditable() ? Integer
					.valueOf(
							rendererGuiConstants.getSpecialProperty(
									mainContainerName + ".Level"
											+ containerLevel
											+ ".editFontSizeEditable",
									Integer.valueOf(
											rendererGuiConstants
													.getEditFontSizeEditable())
											.toString())).intValue() : Integer
					.valueOf(
							rendererGuiConstants.getSpecialProperty(
									mainContainerName + ".Level"
											+ containerLevel
											+ ".editFontSizeReadOnly",
									Integer.valueOf(
											rendererGuiConstants
													.getEditFontSizeReadOnly())
											.toString())).intValue();
			setLookAndFeelFont(FontName, FontStyle, fontSize, component);

			String fontColorStr = rendererGuiConstants.getSpecialProperty(
					mainContainerName + ".Level" + containerLevel
							+ ".editFontColor", null);
			Color fontColor = (fontColorStr == null) ? rendererGuiConstants
					.getEditFontColor() : new Color(Integer.decode("0x"
					+ fontColorStr));
			...
			String FontName = rendererGuiConstants.getSpecialProperty(
					mainContainerName + ".Level" + containerLevel
							+ ".textAreaFont", rendererGuiConstants
							.getTextAreaFont());
			String FontStyle = rendererGuiConstants.getSpecialProperty(
					mainContainerName + ".Level" + containerLevel
							+ ".textAreaFontStyle", rendererGuiConstants
							.getTextAreaFontStyle());
			int fontSize = Integer.valueOf(
					rendererGuiConstants.getSpecialProperty(mainContainerName
							+ ".Level" + containerLevel + ".textAreaFontSize",
							Integer.valueOf(
									rendererGuiConstants.getTextAreaFontSize())
									.toString())).intValue();
			setLookAndFeelFont(FontName, FontStyle, fontSize, component);
			...
			Color fontColor = (fontColorStr == null) ? rendererGuiConstants
					.getTextAreaFontColor() : new Color(Integer.decode("0x"
					+ fontColorStr));
			component.setForeground(fontColor);
			...
			String FontName = rendererGuiConstants.getSpecialProperty(
					mainContainerName + ".Level" + containerLevel
							+ ".sliderFont", rendererGuiConstants
							.getSliderFont());
			String FontStyle = rendererGuiConstants.getSpecialProperty(
					mainContainerName + ".Level" + containerLevel
							+ ".sliderFontStyle", rendererGuiConstants
							.getSliderFontStyle());
			int fontSize = Integer.valueOf(
					rendererGuiConstants.getSpecialProperty(mainContainerName
							+ ".Level" + containerLevel + ".sliderFontSize",
							Integer.valueOf(
									rendererGuiConstants.getSliderFontSize())
									.toString())).intValue();
			setLookAndFeelFont(FontName, FontStyle, fontSize, component);

			String fontColorStr = rendererGuiConstants.getSpecialProperty(
					mainContainerName + ".Level" + containerLevel
							+ ".sliderFontColor", null);
			Color fontColor = (fontColorStr == null) ? rendererGuiConstants
					.getSliderFontColor() : new Color(Integer.decode("0x"
					+ fontColorStr));
			component.setForeground(fontColor);
			...
			String FontName = rendererGuiConstants.getSpecialProperty(
					mainContainerName + ".Level" + containerLevel
							+ ".selectFont", rendererGuiConstants
							.getSelectFont());
			String FontStyle = rendererGuiConstants.getSpecialProperty(
					mainContainerName + ".Level" + containerLevel
							+ ".selectFontStyle", rendererGuiConstants
							.getSelectFontStyle());
			int fontSize = Integer.valueOf(
					rendererGuiConstants.getSpecialProperty(mainContainerName
							+ ".Level" + containerLevel + ".selectFontSize",
							Integer.valueOf(
									rendererGuiConstants.getSelectFontSize())
									.toString())).intValue();
			setLookAndFeelFont(FontName, FontStyle, fontSize, component);
			...
			Color fontColor = (fontColorStr == null) ? rendererGuiConstants
					.getSelectFontColor() : new Color(Integer.decode("0x"
					+ fontColorStr));
			String backgroundColorStr = rendererGuiConstants
					.getSpecialProperty(mainContainerName + ".Level"
							+ containerLevel + ".backgroundColorStr", null);
			Color backgroundColor = (backgroundColorStr == null) ? rendererGuiConstants
					.getSelectBackgroundColor()
					: new Color(Integer.decode("0x" + backgroundColorStr));
			...
			if (rendererGuiConstants.getStretchSelectToMax()
					&& (viewPortWidth > 0))
				component.setPreferredSize(new Dimension(Double.valueOf(
						viewPortWidth - component.getLocation().getX())
						.intValue() - 10, Double.valueOf(
						component.getPreferredSize().getHeight()).intValue()));
			// else //it doesn't work here, so I put it in the renderSelect
			// function.
			// component.setPreferredSize(component.getMinimumSize());
		} else {
			// there are used label font contstants as general font constants.
			// In case they must be different, new general font constants must
			// be added and used there
			String FontName = rendererGuiConstants.getSpecialProperty(
					mainContainerName + ".Level" + containerLevel
							+ ".labelFontName", rendererGuiConstants
							.getLabelFont());
			String FontStyle = rendererGuiConstants.getSpecialProperty(
					mainContainerName + ".Level" + containerLevel
							+ ".labelFontStyle", rendererGuiConstants
							.getLabelFontStyle());
			int fontSize = Integer.valueOf(
					rendererGuiConstants.getSpecialProperty(mainContainerName
							+ ".Level" + containerLevel + ".labelFontSize",
							Integer.valueOf(
									rendererGuiConstants.getLabelFontSize())
									.toString())).intValue();
			setLookAndFeelFont(FontName, FontStyle, fontSize, component);
			...
			String fontColorStr = rendererGuiConstants.getSpecialProperty(
					mainContainerName + ".Level" + containerLevel
							+ ".labelFontColor", null);
			Color fontColor = (fontColorStr == null) ? rendererGuiConstants
					.getLabelFontColor() : new Color(Integer.decode("0x"
					+ fontColorStr));
			component.setForeground(fontColor);
		}
	}

	private void redrawFinalContainer(Container container, int containerLevel,
			String mainContainerName) {
		Component[] containerComponents = container.getComponents();
		String logString = new String("");
		logString += "redrawFinalContainer Container name: "
				+ container.getName();
		...
		String backgroundColorCode = rendererGuiConstants.getSpecialProperty(
				mainContainerName + ".Level" + containerLevel
						+ ".backgroundColor", null);
		Color backgroundColor = ((backgroundColorCode != null) ? new Color(
				Integer.decode("0x" + backgroundColorCode))
				: rendererGuiConstants.getDefaultBackgroundColor());
		container.setBackground(backgroundColor);

		String colsStr = rendererGuiConstants.getSpecialProperty(
				mainContainerName + ".Level" + containerLevel + ".columns",
				null);
		String rowsStr = rendererGuiConstants.getSpecialProperty(
				mainContainerName + ".Level" + containerLevel + ".rows",
				Integer.valueOf(rendererGuiConstants.getDefaultRows())
						.toString());

		int appropriatePanelsPerRow = 0;
		if (rowsStr.equals("1"))
			appropriatePanelsPerRow = containerComponents.length;
		else if (colsStr != null)
			appropriatePanelsPerRow = Integer.valueOf(colsStr);
		else if (container.getName() != null) // if there is not special
			// property set for this
			// container, look for general
			// purpose constants in
			// renderGuiConstants
			if (container.getName().equals(
					rendererGuiConstants.getMainMenuPanelName()))
				appropriatePanelsPerRow = rendererGuiConstants
						.getDefaultMenuCols();
			else // redraw only mainPanel or its childs
			if ((container.getName().equals(rendererGuiConstants
					.getMainPanelPanelName()))
					|| (mainContainerName.equals(rendererGuiConstants
							.getMainPanelPanelName())))
		...
		if ((containerComponents.length > 0) && (appropriatePanelsPerRow > 0)) // this
		// excepts
		// JScrollBar
		// which
		// needs
		...
		// containers
		{
			int appropriatePanelsPerColumn = (int) Math
					.ceil((double) containerComponents.length
							/ (double) appropriatePanelsPerRow);
			container.removeAll();

			TableLayout layout = new TableLayout(getLayoutSize(
					appropriatePanelsPerRow, appropriatePanelsPerColumn));

			container.setLayout(layout);
			int componentIndex = 0;

			logString += " - Container components redrawn: ";
			logString += " - cols X rows = " + appropriatePanelsPerRow + " X "
					+ appropriatePanelsPerColumn;

			Container parent = container.getParent();
			double viewPortWidth = -1;
			if (parent != null) // go back to find the main viewport of one of
				// four basic screen panels. Needed to know the
				// visible width of the main panel
				while (!((parent instanceof JViewport) || (parent instanceof JFrame))) {
					parent = parent.getParent();
				}
			if ((parent instanceof JViewport)
					&& (((JViewport) parent).getParent().getName() != null)
					&& ( // is one of four basic (top, right, bottom, center
					// menu / center application) containers
					(((JViewport) parent).getParent().getName()
							.equals(rendererGuiConstants.getFooterPanelName()))
							|| (((JViewport) parent).getParent().getName()
									.equals(rendererGuiConstants
											.getHeaderPanelName()))
							|| (((JViewport) parent).getParent().getName()
							...
							|| (((JViewport) parent).getParent().getName()
									.equals(rendererGuiConstants
											.getMainMenuPanelName())) || (((JViewport) parent)
							.getParent().getName().equals(rendererGuiConstants
							.getMainPanelPanelName()))))
			...
			for (int rowIndex = 0; rowIndex < appropriatePanelsPerColumn; rowIndex++) {
				for (int colIndex = 0; colIndex < appropriatePanelsPerRow; colIndex++) {
					if (componentIndex < containerComponents.length) {
						...
						String HAlign = rendererGuiConstants
								.getSpecialProperty(
										mainContainerName + ".Level"
												+ containerLevel + ".HAlign",
										rendererGuiConstants.getDefaultHAlign());
						String VAlign = rendererGuiConstants
								.getSpecialProperty(
										mainContainerName + ".Level"
												+ containerLevel + ".VAlign",
										rendererGuiConstants.getDefaultVAlign());

						redrawComponent(viewPortWidth, container, component,
								containerLevel, rowIndex, colIndex,
								componentIndex, container.getName(),
								mainContainerName);
						container.add(component, colIndex * 2 + ", " + rowIndex
								* 2 + ", " + HAlign + ", " + VAlign);
	...
	private void redrawMainMenu(Container container, int containerLevel,
			String mainContainerName) {
		Component[] containerComponents = container.getComponents();
		...
		String colsObj = rendererGuiConstants.getSpecialProperty(
				mainContainerName + ".Level" + containerLevel + ".columns",
				null);
		String rowsStr = rendererGuiConstants.getSpecialProperty(
				mainContainerName + ".Level" + containerLevel + ".rows",
		...
		if (rowsStr.equals("1"))
			...
			appropriatePanelsPerRow = Integer.valueOf(colsObj);
				...
				&& (container.getName().equals(rendererGuiConstants
						.getMainMenuPanelName())))
		...
		// containers
		{
			int appropriatePanelsPerColumn = (int) Math
					.ceil((double) containerComponents.length
							/ (double) appropriatePanelsPerRow);
			// we suppose that in the main menu there is one component (the
			// search panel) which must be managed not in the same way that
			// other components (buttons)
			// as it is shown in the screen, it's better the search component to
			// be placed on the right of the buttons, so one more column is
			// added, plus the border column.
				...
				for (int colIndex = 0; colIndex < appropriatePanelsPerRow - 1; colIndex++) {
					...
					if (componentIndex < containerComponents.length) {
						...
						if ((containerComponents[componentIndex] instanceof JPanel)
								&& ((((JPanel) containerComponents[componentIndex])
										.getName() != null) && (((JPanel) containerComponents[componentIndex])
										.getName().equals(rendererGuiConstants
										.getSearchBoxLabel())))) {
							containerComponents[componentIndex].setSize(
									containerComponents[componentIndex]
											.getWidth() + 10,
									containerComponents[componentIndex]
											.getHeight() + 10);
							containerComponents[componentIndex]
									.setPreferredSize(new Dimension(
											...
											containerComponents[componentIndex]
													.getHeight() + 10));
							container.add(containerComponents[componentIndex],
									(appropriatePanelsPerRow - 1) * 2 + ", "
											+ 0 + ", "
											+ (appropriatePanelsPerRow - 1) * 2
											+ ", " + /*
													 * (appropriatePanelsPerColumn
											...
											+ VAlign);
							// System.err.println(">> " +
							// containerComponents[componentIndex].getClass().getName()
	...
	/**
	 * Implements
	 * {@link Scrollable#getScrollableBlockIncrement(Rectangle, int, int)}
	 */
	...
	/**
	 * Implements
	 * {@link Scrollable#getScrollableUnitIncrement(Rectangle, int, int)}
	 */
	 ...
	 * @return appropriateWidth, appropriateHeight, appropriateCols,
	 *         appropriateRows Checks all the possible compositions in order to
	 *         show x components on a panel in a way that the shape of all
	 *         panels will be analog to the shape of the container panel E.g. if
	 *         we have to show 10 components, it checks which from the
	 *         compositions 2x5, 3x4, 4x3, 5x2 creates a shape having a
	 *         width/height ratio, close to that of the container panel. Taking
	 *         into consideration that the width of each column equals the
	 *         max(width) of its components and the height of each row equals
	 *         the max(height) of its components, there is computed the total
	 *         panel width and height for each of the above compositions
	 */
	// private double[] getAppropriateContainer(Container container, double
	// componentsHorizontalBorder, double componentsVerticalBorder){
	// double returnValues[] = new double[4];
	// Component[] components = container.getComponents();
	// int compositions = components.length/2;
	// compositions = ((compositions*2) <
	// components.length)?compositions:compositions--; //if 10 components,
	// (10/2)-1 compositions. if 11 components, 11/2 compositions
	// compositions++;
	...
	// }
	// if (parent instanceof JViewport)
	// panelDimRatio = ((JViewport)parent).getViewRect().getWidth() /
	// ((JViewport)parent).getViewRect().getHeight();
	// //panelDimRatio = ((JViewport)panel.getParent()).getViewRect().getWidth()
	// / ((JViewport)panel.getParent()).getViewRect().getHeight();
	// int appropriateCols = 0;
	// int appropriateRows = 0;
	//		
	// for (int colCount=2;colCount<=compositions;colCount++) //all available
	// compositions : from 2 to components/2 or components/2 + 1
	// {
	// double compositionWidth = 0;
	// double compositionHeight = 0;
	// double compositionDimRatio = 0;
	//			
	// for (int compCol=0;compCol<colCount;compCol++) //max width of all columns
	// e.g. maxWidth(01,03,05,07,09) + maxWidth (02,04,06,08,10)
	// {
	// double maxComponentWidth = 0;
	// for (int compRowCol=compCol; compRowCol<components.length; compRowCol +=
	// colCount) //max width of current column. e.g. maxWidth(01,03,05,07,09)
	// {
	// maxComponentWidth = (maxComponentWidth >
	// components[compRowCol].getSize().getWidth())?maxComponentWidth:components[compRowCol].getSize().getWidth();
	// }
	...
	// components.length)?rowCount++:rowCount;
	//			
	// for (int compRow=0;compRow<rowCount;compRow++) //max height of all rows
	// e.g. maxHeight(01,02) + maxHeight(03,04) + maxHeight(05,06) +
	// maxHeight(07,08) + maxHeight(09,10)
	// {
	// double maxComponentHeight = 0;
	// for (int compColRow=compRow*colCount; compColRow<colCount*(compRow+1);
	// compColRow++)
	// {
	// maxComponentHeight = (maxComponentHeight >
	// components[compColRow].getSize().getHeight())?maxComponentHeight:components[compColRow].getSize().getHeight();
	// }
	// compositionHeight += maxComponentHeight + componentsVerticalBorder;
	// }
	// compositionDimRatio = compositionWidth/compositionHeight;
	// if (Math.abs(panelDimRatio - appropriateRatio) > Math.abs(panelDimRatio -
	// (compositionDimRatio)))
	// {
