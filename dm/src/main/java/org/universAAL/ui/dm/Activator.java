...
//		Resource b = new Resource(Constants.uAAL_MIDDLEWARE_LOCAL_ID_PREFIX + "boiler1");
//		b.addType("http://ontology.persona.ratio.it/DummyServiceProvider.owl#Boiler", true);
//		Location pl = new Location(Constants.uAAL_MIDDLEWARE_LOCAL_ID_PREFIX + "livingRoom");
//		b.setProperty(PhysicalThing.PROP_PHYSICAL_LOCATION, pl);
//		insert(b);
//		Temperature t = new Temperature();
//		t.setValue(21);
//		pl.setTargetTemperature(t);
//		new DefaultContextPublisher(context, null).publish(new ContextEvent(pl, Location.PROP_TARGET_TEMPERATURE));
//	}
	...
	static DBConnection getConnection() {
		return new DBConnection(JENA_DB_URL, JENA_DB_USER, JENA_DB_PASSWORD, "MySQL"); 
	}
	...
	public void run() {
		try {
			...
			LogUtils.logError(logger, "Activator", "start", new Object[]{"Cannot initialize Dialog Manager externalized strings!"}, e);
			return;
		}
		
		contextSubscriber = new ContextSubscriber(context);
		// it is important to instantiate the input subscriber before the output publisher
	...
	public void start(BundleContext context) throws Exception {
		...
		ServiceReference sref = context.getServiceReference(MessageContentSerializer.class.getName());
		serializer = (sref == null)? null : (MessageContentSerializer) context.getService(sref);
