...
/**
 * The mechanism to update the {@link UIPreferencesSubProfile} is though the Context bus,
 * when an update is needed outside the UI.DM just publish an event whith the {@link UIPreferencesSubProfile}
 * as object. 
 * @author amedrano
 *
 */
public class UIPreferencesBufferSubscriptor extends ContextSubscriber
	implements IUIPreferencesBuffer {
	...
	subscriptions[0].addRestriction(
		MergedRestriction
		.getAllValuesRestrictionWithCardinality(ContextEvent.PROP_RDF_OBJECT,
			UIPreferencesSubProfile.MY_URI, 1, 1));
    }
     ...
     */
    private UIPreferencesBufferSubscriptor(ModuleContext connectingModule,
