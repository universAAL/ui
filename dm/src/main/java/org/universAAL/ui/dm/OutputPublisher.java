...
public class OutputPublisher extends
	...
	try {
		...
		ModelRDB CHModel = ModelRDB.open(con,
			SharedResources.JENA_MODEL_NAME);
	    ...
	    LogUtils.logError(SharedResources.moduleContext, this.getClass(),
		    "addAdaptationParams", null, e); //$NON-NLS-1$ //$NON-NLS-2$
	}
    }
	...
	case DialogType.STD_DIALOG:
	    ...
	    if (!SharedResources.getString("OutputPublisher.pendingMessages")
		    .equals(dialogTitle))
		new Submit(stdButtons, new Label(SharedResources
			.getString("OutputPublisher.pendingMessages"), null),
			MESSAGES_CALL);
	...
	synchronized (waitingDialogs) {
		...
		if (finished == null) {
			...
			LogUtils.logWarn(SharedResources.moduleContext, this
	...
	return SharedResources.getString("OutputPublisher.noPendingMessages")
		.equals(msgContent)
	...
	synchronized (waitingDialogs) {
		...
		// message to the user
		pushDialog(
			out.getAddressedUser(),
			Form
				.newMessage(
					SharedResources
						.getString("OutputPublisher.forcedCancellation"),
					SharedResources
						.getString("OutputPublisher.sorryAborted")));
	    if (out != null)
	...
	new Submit(g, new Label(SharedResources
		.getString("OutputPublisher.search"), null), SEARCH_CALL)
		.addMandatoryInput(in);
	pushDialog(u, f);
    }
