...
public class ResourceServer extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (reqURI.length() == Activator.URI.length()) {
			...
			LogUtils.logInfo(moduleContext, this.getClass(), "doGet/Post", new Object[] { "Request URI is null." },
					null);
			return;
		}
		...
		LogUtils.logInfo(moduleContext, this.getClass(), "doGet/Post",
				new Object[] { "Request for a following resource received: ", requestedResource }, null);

		// check authorization
		if (!isAuthorized(req, resp)) {
			LogUtils.logInfo(moduleContext, this.getClass(), "doGet/Post",
					new Object[] { "Received unauthorized request for Resource." }, null);
		...
		if (mimeType == null) {
			LogUtils.logInfo(moduleContext, this.getClass(), "doGet/Post",
					new Object[] { "Could not get MIME type of: ", requestedResource }, null);
		...
		if (!requestedResource.startsWith(System.getProperty("file.separator"))) {
			pathToRequestedResource = PATH_TO_RESOURCES + System.getProperty("file.separator") + requestedResource;
		}
		...
		if (!file.exists()) {
			LogUtils.logError(moduleContext, this.getClass(), "doGet/Post",
					new Object[] { "Requested file not found: ", pathToRequestedResource }, null);
		...
		LogUtils.logInfo(moduleContext, this.getClass(), "doGet/Post",
				new Object[] { "Following resource sent: ", pathToRequestedResource }, null);
		in.close();
		out.close();
