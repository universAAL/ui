...
/**
 * Advanced Image display. This component will scale, center and maintain within
 * the maximum size the image.
 * 
 * @author amedrano
 *
 */
public class ImageMedia extends JPanel {
	...
	public void setSize(int width, int height) {
		if (width > 0 && height > 0) {
			...
			if (maxDim != null && width >= maxDim.width) {
				iconSize.width = maxDim.width;
			}
			if (maxDim != null && maxIconHeight >= maxDim.height) {
				iconSize.height = maxIconHeight;
			}
			if (minDim != null && width < minDim.width) {
				iconSize.width = minDim.width;
			}
			if (minDim != null && maxIconHeight < minDim.height) {
				iconSize.height = minDim.height;
			}
			if (originalSize.width > originalSize.height) {
				iconSize.height = (iconSize.width / originalSize.width) * originalSize.height;
			}
			if (originalSize.width > originalSize.height) {
				iconSize.width = (iconSize.height / originalSize.height) * originalSize.width;
			}
			if (originalSize.width == originalSize.height) {
