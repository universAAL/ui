...
public class ComponentBorder extends AbstractBorder {
	 ...
	 */
	public ComponentBorder(Border border, Component title, int titleJustification, int titlePosition) {
	...
	public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
		...
		case DEFAULT_POSITION:
			...
			diff = fontHeight / 2;
			grooveRect.height -= diff;
			textLoc.y = grooveRect.y + insets.top / 2;
			grooveRect.y += diff;
			break;
		case BELOW_TOP:
			...
			break;
		case ABOVE_BOTTOM:
			textLoc.y = (grooveRect.y + grooveRect.height) - (insets.bottom + TEXT_SPACING);
			break;
		case BOTTOM:
			grooveRect.height -= fontHeight / 2;
			textLoc.y = (grooveRect.y + grooveRect.height) + insets.bottom / 2;
			break;
		case BELOW_BOTTOM:
			grooveRect.height -= fontHeight;
			textLoc.y = grooveRect.y + grooveRect.height + TEXT_SPACING;
			break;
		}
		...
		if (border != null) {
			if (((titlePos == TOP || titlePos == DEFAULT_POSITION) && (grooveRect.y > textLoc.y))
				...
				if (titlePos == TOP || titlePos == DEFAULT_POSITION) {
					...
					clipRect.setBounds(saveClip);
					if (computeIntersection(clipRect, textLoc.x - 1, textLoc.y, stringWidth + 2,
							y + height - textLoc.y)) {
						g.setClip(clipRect);
						border.paintBorder(c, g, grooveRect.x, grooveRect.y, grooveRect.width, grooveRect.height);
					}
					...
					clipRect.setBounds(saveClip);
					if (computeIntersection(clipRect, textLoc.x - 1, y, stringWidth + 2, textLoc.y - y)) {
						g.setClip(clipRect);
						border.paintBorder(c, g, grooveRect.x, grooveRect.y, grooveRect.width, grooveRect.height);
					}
				}
	...
	public int getBaseline(Component c, int width, int height) {
		...
		if (title != null) {
			...
			switch (getTitlePosition()) {
			case ABOVE_TOP:
				diff = Math.max(EDGE_SPACING, TEXT_SPACING * 2) - EDGE_SPACING;
				return y + diff - TEXT_SPACING;
			case TOP:
			case DEFAULT_POSITION:
				diff = Math.max(0, TEXT_SPACING - EDGE_SPACING);
				return (y + diff) + borderInsets.top / 2;
			case BELOW_TOP:
				return y + borderInsets.top + TEXT_SPACING;
			case ABOVE_BOTTOM:
				return (y + h) - (borderInsets.bottom + TEXT_SPACING);
			case BOTTOM:
				h -= fontHeight / 2;
				return (y + h) + borderInsets.bottom / 2;
			case BELOW_BOTTOM:
				h -= fontHeight;
	...
	public static void addLabeledBorder(JLabel label, JComponent jc, ColorLAF color) {
		if (label != null) {
			// resize Icon
			Icon icon = IconFactory.resizeIcon(label.getIcon(), color.getLabelIconSize(), color.getLabelIconSize());
			label.setIcon(icon);
			...
			jc.setBorder(BorderFactory.createLineBorder(color.getOrange()));
		}
	}
}
