...
public final class Handler extends UIHandler {
	...
	Handler(Renderer renderer) {
		super(renderer.getModuleContext(), getPermanentSubscriptions(
				Boolean.parseBoolean(renderer.getProperty(Renderer.DEMO_MODE)), renderer.getRendererLocation()));
		render = renderer;
	}
	...
	public void unSetCurrentUser() {
		if (currentUser != null) {
			UIHandlerProfile oep = getPermanentSubscriptions(
					Boolean.parseBoolean(render.getProperty(Renderer.DEMO_MODE)), render.getRendererLocation());
			oep.addRestriction(MergedRestriction.getFixedValueRestriction(UIRequest.PROP_ADDRESSED_USER, currentUser));
	...
	private void userAuthenticated(User user) {
		 ...
		 */
		UIHandlerProfile oep = getPermanentSubscriptions(Boolean.parseBoolean(render.getProperty(Renderer.DEMO_MODE)),
	...
	public void submit(Submit submit) {
		if (currentUser != null && render != null && submit != null) {
			dialogFinished(new UIResponse(currentUser, render.getRendererLocation(), submit));
		} else {
