...
public final class Activator implements BundleActivator, IContainerManager {
	...
	public void start(BundleContext context) throws Exception {
		...
		if (props != null && props.length > 0) {
			...
			for (int i = 0; i < props.length; i++) {
				...
				// new Object[] {
				// getPropName(props[i]),
				// "Configure and instance in the UI.Handler.gui.swing
				// component",
		...
		/*
		 * Vadim - put renderer creation in a separate thread, see a discussion
		 * here -
		 * http://forge.universaal.org/gf/project/uaal_ui/forum/?_forum_action=
		 * ForumMessageBrowse&thread_id=91&action=ForumBrowse&forum_id=89
		 */
	...
	// d.put("queued.forms",
	// "The Class which handles incoming dialogs. It can be either of these
	// three options:\n"
	...
	// +
	// " dialogs are processed one by one, but kept when subdialogs come in so
	// the full hierarchy of dialogs is available.");
	// d.put("LookandFeel.package", "The look and feel package to be used (if
	// not found, defaultLAF package will be used)");
	// d.put("demo.mode", "Configures the Handler in demo Mode, disables
	// Impairment register restrictions (default=true).");
	// d.put("default.user", " if set, the handler automatically logs in for
	// this user, if not (default) user will be asked to provide credentials.");
	// return d;
	// }
	//
	...
	public void shutdownContainer() {
		try {
			...
			LogUtils.logWarn(context, getClass(), "shutdownContainer", new String[] { "Unable to shutdown container." },
					e);
		}
	}
}
